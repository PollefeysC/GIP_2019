...
if (isset($_POST['newpassword']) && isset($_POST['customerID'])) {
    ...
    if ($connect1->connect_error) {
        ...
        if ($connect1->query($sql) === TRUE) {
            echo "<h2>Your password was updated!";
            echo " You can now log in!</h2>";
        } else {
            echo "<h3>Error updating password: " . $connect1->error . "</h3>";
        }
    }
    ...
    if (isset($_POST["email"])) {
        if ($connect1->connect_error) {
            ...
            if ($qresult->num_rows > 0) {
                while ($endresult = $qresult->fetch_assoc()) {
                    ...
                    $onderwerp = "Pegasus TT: recover your password";
                    $bericht = "Your account requested a password reset. \n";
                    $bericht .= "If you did not request this, please ignore this email. \n\n";
                    $bericht .= "You can reset your password at: ".$link;

                    echo "<h2>We have sent you a recovery email</h2>";
        ...
        ?>
        <h2>Please tell us your email</h2>
        <form method="POST" action="../register/recover.php" id="registrationform">
