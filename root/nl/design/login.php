...
<div id="overlay">
    ...
    <form method="POST" name="form" id="loginform">
        <table id="logintable">
            ...
            <tr>
                <td id="registreertd"> Geen account? <a href="#" onclick="changeToSignup()">Registreer!</a> </td>
            </tr>
            <tr>
                <td id="registreertd"> Wachtwoord vergeten? <a href="../register/recover.php">Stuur Recovery Mail</a> </td>
            </tr>
    ...
    <form method="POST" name="form" id="registerform">
        <table id="registertable">
            ...
            <tr>
                <td><input type="text" placeholder="Naam" name="regFullName" class="registerField regFullName" id="fullNameReg"> </td>
            </tr>

            <tr>
                <td><input type="text" placeholder="Straat" name="regStreet" class="registerField regStreet" id="streetReg">
                <input type="text" placeholder="Nummer" name="regNumber" class="registerField regNumber" id="numberReg">
                <input type="text" placeholder="Postcode" name="regPostalCode" class="registerField regPostalCode" id="PostalCodeReg"> </td>
            </tr>
            ...
            <tr>
                <td><input type="password" placeholder="Wachtwoord" name="regPassword" class="registerField regPassword" id="passwordReg">
            </tr>
            ...
            <tr>
                <td><input type="submit" id="submit" name="sendsignup" value="Registreer" class="registerField"></td>
            </tr>

            <tr>
                <td id="logintd"> Bestaande gebruiker? <a href="#" onclick="changeToLogin()">Login!</a> </td>
            </tr>
...
if (isset($_POST["sendsignup"])) {
    if (!empty($errorMsg)) {
        echo "<script> alert('Je maakte de volgende fouten in je registratie, gelieve opnieuw te proberen: ".$errorMsg."');</script>";
    } else {
        ...
        if ($qresult->num_rows > 0) {
            echo "<script> alert('Deze email heeft al een gelinkte gebruiker'); </script>";
        } else {
            //maak account aan
    ...
    if ($qresult3->num_rows > 0) {
        while ($endresult = $qresult3->fetch_assoc()) {
            if ($endresult["verified"] == 1) {
                ...
                echo "<script> alert('Deze email is nog niet bevestigd'); </script>";
            }
        }
    } else {
        echo "<script> alert('Deze combinatie van email en wachtwoord klopt niet.'); </script>";
    }
}


function sendMail($reg_name, $reg_code){
    echo "<script> alert('Email wordt verstuurd... Gelieve uw inbox te bekijken om verder te gaan'); </script>";
    ...
    $reg_site = "http://localhost:8080/nl/register";
    $onderwerp = "Pegasus TT: activeer uw account";
    $bericht = "Bedankt voor uw registratie \n\n";
    $bericht .= "Uw naam is: $reg_name\n";
    $bericht .= "Uw verificatiecode is: $reg_code\n";
    $bericht .= "Gelieve deze code te plakken in het veld op de volgende website: ".$reg_site ."\n";
    $bericht .= "Of gebruik deze directe link: ".$reg_site."/?verification_key=".$reg_code;
